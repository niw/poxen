...
Puppet::Type.type(:package).provide(:appdmg_eula, :parent => Puppet::Provider::Package) do
  desc "Package management which copies application bundles which require acceptance of an EULA to a target."
  ...
  def self.url_regexp
    @url_regexp ||= %r{\A[A-Za-z][A-Za-z0-9+\-\.]*://}
  end

  def self.installpkgdmg(source, name)
    unless source =~ /\.dmg$/i || source =~ url_regexp
